...
describe(`gatsby-plugin-giffit`, () => {
  ...
  describe(`queueImageResizing`, () => {
    it(`should round height when auto-calculated`, () => {
      // Resize test.gif (55x55) with a 3px width
      const result = queueImageResizing({
        file: getFileObject(path.join(__dirname, `images/test.gif`)),
        args: { width: 3 },
      })

      // Width should be: w = (3 * 55) / 55 = 3
      // We expect value to be rounded to 3
    ...
    it(`file name works with spaces & special characters`, async () => {
      ...
      const queueResult = queueImageResizing({
        file: getFileObject(
          path.join(__dirname, `images/run-bullet.gif`),
          testName
        ),
        args: { width: 3 },
      })
  ...
  //     // add the original size of `test.gif`
